...
public class SolverForQueensPuzzle {
    ...
    private void recordSolutionsStarted() {
			if(inProgress.accept()){ //One Base Case
				...
				for(int i = 0; i<inProgress.ranks();i++){
					...
					if(inProgress.lastIsNg()) //Invalid placement
						inProgress.depopulate(); //Time to go back
					else
						recordSolutionsStarted(); //Time to go forward
					nBoardsConsidered++; //Each placement is a board considered
				}	
			}
			inProgress.depopulate(); //A queen placed in each file, go back to previous rank
